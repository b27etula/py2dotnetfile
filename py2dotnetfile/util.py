...
def convert_to_unicode(byte_string):
    ...
    for encoding in UTF8_ENCODINGS:
        try:
            unicode_string = str(byte_string).decode(encoding)
...
def get_reasonable_display_string_for_bytes(string_bytes):
    """
    ...
    if display_string is None:
        display_string = '(hex){}'.format(binascii.hexlify(string_bytes))

    return display_string


def read_null_terminated_byte_string(byte_buffer, limit=128):
    """
...
def read_reasonable_string(byte_buffer, limit=128):
    """
    ...
    for b in search_buffer:
        ...
        null_terminated_string += bytearray(b, 'utf-8')
...
def bytes_to_annotated_hex_string(byte_buffer):
    """
    Supplies the string buffer used for json dumps, etc... when we have weird strings.
    """
    hex_string = ''.join(['{:02x}'.format(ord(x)) for x in byte_buffer])
    ascii_string = bytes_to_ascii(byte_buffer)
    annotated_hex_string = '0x{:x} bytes: (HEX){} - (ASCII){}\n'.format(len(byte_buffer), hex_string, ascii_string)
    return annotated_hex_string


def make_string_readable(field_string):
    """
...
def read_7bit_encoded_uint32(byte_input):
    """
...
def read_7bit_encoded_int32(byte_input):
    """
...
class FileLocation(object):
    ...
    """
    def __init__(self, address=None, string_representation=None, size=None):
    ...
    def string_representation(self):
        if self.__string_representation is None:
            ...
            if isinstance(given_argument, bytes):
                try:
                    self.__string_representation = given_argument.decode('utf-8')
                except UnicodeDecodeError:
    ...
    def __str__(self):
        if self.string_representation is not None:
            return '{} (0x{:x})'.format(self.string_representation, self.address)
        else:
            return '(0x{self.address:x})'.format(self.address)
...
class BinaryStructure(object):
    ...
    """
    def __init__(self, addr, display_name, byte_buffer):
        ...
        try:
            self.string_representation = byte_buffer.decode('utf-8')
        except UnicodeDecodeError:
            self.string_representation = str(byte_buffer)

    def create_field_value(self, display_name, length, format_str):
        """
        ...
        # value_bytes = value_bytes.rstrip('\x00')
    ...
    def set_byte_buffer_size(self, byte_buffer_size):
        """
...
class BinaryStructureField(FileLocation):
    ...
    def __init__(self, addr, display_name, format_str, value_bytes, offset):
