...
class MetadataTable(object):
    ...
    """
    def __init__(self, table_rows, addr = None, string_representation = None, size = None):
        self.address = addr
...
class DotNetPEParser(PE):
    ...
    def __init__(self, file_ref, parse=True, *args, **kwargs):
    ...
    def is_metadata_header_complete_and_valid(self, file_ref):
        """
    ...
    def get_clr_header(self):
        ...
        if self.dotnet_anti_metadata['data_directory_hidden']:
            ...
            optional_header_offset = self.NT_HEADERS.get_file_offset() + 4 + self.FILE_HEADER.sizeof()
            section_offset = optional_header_offset + self.FILE_HEADER.SizeOfOptionalHeader
            last_offset_data_dir = self.OPTIONAL_HEADER.DATA_DIRECTORY[number_of_rva_and_sizes - 1].get_file_offset() + 8
    ...
    @property
    def dotnet_headers_are_valid(self):
    ...
    @property
    def metadata_dir_size(self):
        return self.clr_header.MetaDataDirectorySize.value

    def parse_dotnet_stream_headers(self):
        metadata_stream_rva = self.clr_header.MetaDataDirectoryAddress.value
    ...
    def parse_tilde_stream(self, stream_name):
        ...
        self.dotnet_field_size_info = self.calculate_field_size_info(self.dotnet_metadata_stream_header.table_size_lookup)
    ...
    def parse_all_metadata_tables(self, tables_rva):
        """
        ...
        for table_name in self.dotnet_metadata_stream_header.table_names:
            try:
                ...
                if metadata_table is not None:
                    ...
                    raise Exception('table not parsed correctly: {}'.format(table_name))

            except Exception as e:
                error_string = 'Error attempting to parse metadata table: {}'.format(table_name)
                # self.logger.info(error_string)
                # self.logger.exception(e)
                raise Exception(error_string)

    def parse_metadata_table_row_size(self, table_row_addr, row_type):
        """
    ...
    def parse_metadata_table(self, table_name, table_rva, num_rows):
        """
    ...
    def _get_all_string_references(self):
        result = set()
    ...
    def _get_non_standard_strings(self, string_type, string_references, all_strings_data):
    ...
    @staticmethod
    def _get_stream_sequence_length(length_field_buffer):
        # String length explanation:
        # https://www.ecma-international.org/wp-content/uploads/ECMA-335_6th_edition_june_2012.pdf
        # II.24.2.4 #US and #Blob heaps (page 298 or 272)

        # one byte of length
        first_byte = struct.unpack('B', length_field_buffer[0])[0]
    ...
    @staticmethod
    def get_max_rows(table_size_lookup, table_names):
        max_rows = 0
    ...
    def get_field_size_info(self, table_size_lookup, table_names, encoding_bits):
    ...
    def calculate_field_size_info(self, table_size_lookup):
        field_size_info = {}
    ...
    def _resources_exist(self):
        ...
        if self.clr_header.ResourcesDirectoryAddress.value and 'ManifestResource' in self.dotnet_metadata_stream_header.table_names:
            return True

        return False

    @staticmethod
    def _read_serialized_string(string, encoding = 'utf-8'):
        result = ''
    ...
    def _read_resource_data(self, start_rva, size, resource_type):
    ...
    def parse_dotnet_resources(self):
        """
        ...
        if self._resources_exist():
            ...
            for i, table_row in enumerate(resource_table_rows):
                ...
                if next_i < len(resource_table_rows):
                    resource_end_rva = self.clr_header.ResourcesDirectoryAddress.value + resource_table_rows[next_i].Offset.value
                else:
                    resource_end_rva = self.clr_header.ResourcesDirectoryAddress.value + self.clr_header.ResourcesDirectorySize.value
                    ...
                    if resource_reader_header_version == 2:
                        ...
                        if number_types:
                            for j in range(number_types):
                                ...
                                try:
                                    current_string_length_size, current_string_length = read_7bit_encoded_int32(self.get_data(current_string_offset, 4))
                                    current_string = self.get_data(current_string_offset + current_string_length_size, current_string_length).decode('utf-8')
                                ...
                                current_string_offset = current_string_offset + current_string_length_size + current_string_length
                        ...
                        #self.logger.info(f'.NET resource with reader header version {resource_reader_header_version} not supported.')
                        pass
                else:
