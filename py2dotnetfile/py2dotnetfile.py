...
class Type:
    ...
    class MethodDefMemberAccess(IntEnum):
        """
        Sources:
        https://www.ecma-international.org/publications-and-standards/standards/ecma-335/
        https://docs.microsoft.com/en-us/dotnet/api/system.reflection.methodattributes?view=net-5.0
    ...
    class TypeDefVisibility(IntEnum):
        """
        Sources:
        https://www.ecma-international.org/publications-and-standards/standards/ecma-335/
        https://docs.microsoft.com/en-us/dotnet/api/system.reflection.typeattributes?view=net-5.0
...
class Struct:
    class NativeEntryPoint:
        def __init__(self, entrypointtype=None, address=None):
            self.EntryPointType = entrypointtype
            self.Address = address

    class ManagedEntryPoint:
        def __init__(self, entrypointtype=None, method=None, type_=None, namespace='', signature=''):
            self.EntryPointType = entrypointtype
            self.Method = method
            self.Type = type_
    ...
    class TypesMethods:
        def __init__(self, type_=None, namespace='', methods=None, flags=None):
            self.Type = type_
            self.Namespace = namespace
    ...
    class Methods:
        def __init__(self, name='', signature=None, flags=None):
            self.Name = name
            self.Signature = signature
            self.Flags = flags

    class EntryPoint:
        def __init__(self, method=None, signature=None, type_=None, namespace=''):
            self.Method = method
            self.Signature = signature
            self.Type = type_
            self.Namespace = namespace
    ...
    class AssemblyInfo:
        def __init__(self, majorversion=0, minorversion=0, buildnumber=0, revisionnumber=0):
...
class DotNetPE(DotNetPEParser):
    def __init__(self, path):
        super(DotNetPE, self).__init__(path)
    ...
    def metadata_table_exists(self, name):
        """
    ...
    def has_native_entry_point(self):
        """
    ...
    def get_string(self, string_address):
        """
    ...
    @staticmethod
    def get_hash(hash_type, string_list):
        """
    ...
    def get_strings_stream_strings(self):
        """
    ...
    def get_user_stream_strings(self):
        """
    ...
    def get_runtime_target_version(self):
        """
    ...
    def has_invalid_typeref_entries(self):
        ...
        for table_row in self.dotnetpe.metadata_tables_lookup['TypeRef'].table_rows:
            ...
            if type_name == 'TypeRef':
                ...
                if table_row_reference > len(self.dotnetpe.metadata_tables_lookup['TypeRef'].table_rows) or not type_name_address:
                    result = True
                    break
...
class Cor20Header:
    ...
    def get_header_entry_point(self):
        ...
        if (self.dotnetpe.clr_header.Flags.value & flag_native_entry_point) == flag_native_entry_point:
            return Struct.NativeEntryPoint(Type.EntryPoint.NATIVE.value, hex(self.dotnetpe.clr_header.EntryPointToken.value))
        ...
        metadata_table_row = self.dotnetpe.metadata_tables_lookup[METADATA_TABLE_INDEXES[table_index]].table_rows[table_row - 1]
...
class TypeRef:
    ...
    def get_typeref_hash(self, hash_type = Type.Hash.SHA256, skip_self_referenced_entries = True, strings_sorted = False):
        """
        ...
        for current_row_index, table_row in enumerate(self.dotnetpe.metadata_tables_lookup['TypeRef'].table_rows):
            ...
            types_scopes.append('{}-{}'.format(resolution_scope_name, type_name))
...
class TypeDef:
    ...
    def get_type_names(self, visibility = Type.TypeDefVisibility.ANY):
        """
    ...
    def get_type_names_with_methods(self):
        """
...
class MethodDef:
    ...
    def get_method_names(self, method_access = Type.MethodDefMemberAccess.ANY):
        """
    ...
    def get_windows_forms_app_entry_point(self):
        """
        ...
        for table_row in self.dotnetpe.metadata_tables_lookup['CustomAttribute'].table_rows:
            if table_row.table_references['Type'][0] == 'MemberRef':
                ...
                member_ref_parent_table = self.dotnetpe.metadata_tables_lookup['MemberRef'].table_rows[member_ref_index].table_references['Class'][0]

                if member_ref_parent_table == 'TypeRef':
                    typeref_table_index = self.dotnetpe.metadata_tables_lookup['MemberRef'].table_rows[member_ref_index].table_references['Class'][1] - 1
                    ...
                    if method.Flags & Type.MethodDefMask.MEMBERACCESS == 6:
                        if method.Name not in ['.ctor', '.cctor']:
                            if not ((method.Flags & Type.MethodDefMask.SPECIALNAME == Type.MethodDefMask.SPECIALNAME and
                                ...
                                result.append(Struct.EntryPoint(method.Name, method.Signature, general_type.Type, general_type.Namespace))
...
class MemberRef:
    ...
    def get_memberref_names(self, deduplicate = False):
        """
    ...
    def get_memberref_hash(self, hash_type = Type.Hash.SHA256, strings_sorted = False):
        """
        ...
        for current_row_index, table_row in enumerate(self.dotnetpe.metadata_tables_lookup['MemberRef'].table_rows):
            ...
            tables_names.append('{}-{}'.foramt(class_table, name_string))
...
class ModuleRef:
    ...
    def get_unmanaged_module_names(self, modules_type = Type.UnmanagedModules.RAW):
        """
            ...
            if module_name:
                if modules_type == Type.UnmanagedModules.RAW:
                    ...
                    if not module_name.endswith('.dll'):
                        module_name = '{}.dll'.format(module_name)
...
class ImplMap:
    ...
    def _get_function_name_from_methoddef_table(self, table_index):
        result = ''
        ...
        string_address = self.dotnetpe.metadata_tables_lookup['MethodDef'].table_rows[table_index].string_stream_references['Name']
        result = self.dotnetpe.get_string(string_address)

        return result

    def get_unmanaged_functions(self):
        """
        ...
        for table_row in self.dotnetpe.metadata_tables_lookup['ImplMap'].table_rows:
            ...
            if not function_name:
                function_name = self._get_function_name_from_methoddef_table(table_row.table_references['MemberForwarded'][1] - 1)

            result.append(function_name)

        return result

    def get_platform_invoke_information(self, with_mapping_flags = False):
        result = []
        ...
        for table_row in self.dotnetpe.metadata_tables_lookup['ImplMap'].table_rows:
            ...
            if not function_name:
                ...
                module_string_address = self.dotnetpe.metadata_tables_lookup['ModuleRef'].table_rows[import_scope - 1].string_stream_references['Name']
            ...
            if with_mapping_flags:
                ...
                result.append('{module_name}.{function_name}.{mapping_flags}'.format(module_name, function_name, mapping_flags))
            else:
                result.append('{module_name}.{function_name}'.format(module_name, function_name))
...
class Assembly:
    ...
    def get_assembly_culture(self):
        """
        Get assembly culture.
        """
        # To counteract .NET protectors like ConfuserEx that add additional entries, we skip the remaining row(s) as
        # they are officially not supported and contain invalid entries
        string_address = self.dotnetpe.metadata_tables_lookup['Assembly'].table_rows[0].string_stream_references['Culture']

        return self.dotnetpe.get_string(string_address)

    def get_assembly_version_information(self):
        """
...
class AssemblyRef:
    ...
    def get_assemblyref_names(self, deduplicate = False):
        """
    ...
    def get_assemblyref_cultures(self):
        """
