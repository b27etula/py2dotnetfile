...
class DOTNET_CLR_HEADER(BinaryStructure):
    def __init__(self, addr=None, byte_buffer=None):
...
class DOTNET_METADATA_HEADER(BinaryStructure):
    def __init__(self, addr=None, byte_buffer=None):
        ...
        if self.VersionStringLength.value > 0x100:
            raise Exception('Invalid .NET metadata Version String length: 0x{:x}'.format(self.VersionStringLength.value), addr)

        version_string_struct_string = '{:d}s'.format(self.VersionStringLength.value)
        self.VersionString = self.create_field_value('VersionString', self.VersionStringLength.value, version_string_struct_string)
...
class DOTNET_STREAM_HEADER(BinaryStructure):
    def __init__(self, addr=None, byte_buffer=None):
        ...
        name_struct_string = '{:d}s'.format(name_len)

        self.Name = self.create_field_value('Name', name_len, name_struct_string)
        self.string_representation = '{} Stream Header'.format(name)
...
class DOTNET_METADATA_STREAM_HEADER(BinaryStructure):
    def __init__(self, addr=None, byte_buffer=None):
        ...
        for metadata_table_flag in sorted(self.metadata_table_flags.keys()):
            ...
            table_size_location = self.create_field_value('TableSize_{}'.format(table_name), 4, 'I')
